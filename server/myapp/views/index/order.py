...
def create(request):
    ...
    if data['user'] is None or data['thing'] is None or data['count'] is None or data['receiver_time'] is None:
        return APIResponse(code=1, msg='参数错误')

    orders = Order.objects.filter(thing_id=data['thing'], receiver_time=data['receiver_time'], status='1')
    if orders and len(orders) > 0:
        return APIResponse(code=1, msg='该场次已被其他用户预约')
